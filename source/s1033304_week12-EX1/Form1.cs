...
namespace s1033304_week12_EX1
{
    public partial class Form1 : Form
    {
        ...
        Brush b;
        Color BrushC = Color.White;
        Boolean Brushornot = false;
                ...
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    else if(tools==2)
                    {
                        if ((e.X < w) && (e.Y < h) && (e.X > 0) && (e.Y > 0))
                        {
                            ...
                            if(Brushornot)
                            {
                                gg.FillRectangle(b, Math.Min(x0, e.X), Math.Min(y0, e.Y), Width, Height);
                                gg.DrawRectangle(p, Math.Min(x0, e.X), Math.Min(y0, e.Y), Width, Height);
                            }
                            else
                            {
                                gg.DrawRectangle(p, Math.Min(x0, e.X), Math.Min(y0, e.Y), Width, Height);
        ...
        private void 顏色ToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (colorDialog1.ShowDialog() == DialogResult.OK)
            {
                BrushC = colorDialog1.Color;
                b = new SolidBrush(BrushC);
                toolStripStatusLabel4.Text = "Brush Color: " + b.ToString();
            }
        }
        ...
        {
            Brushornot = true;
            填充ToolStripMenuItem.Checked = true;
            無填充ToolStripMenuItem.Checked = false;
        }

        private void 無填充ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Brushornot = false;
            填充ToolStripMenuItem.Checked = false;
            無填充ToolStripMenuItem.Checked = true;
        }
